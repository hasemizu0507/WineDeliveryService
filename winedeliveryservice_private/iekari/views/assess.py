...
from iekari.scripts.predict_price import PredictPrice

from iekari.models import RentRoom

def form_view(request): # フォームを表示
    return render(request, 'iekari/templates/iekari/assessment_form.html')  #　変更
...
def result_view(request): # フォームの入力をAPIに伝達、リターン値を用いてテンプレートに表示
    ...
    predict_price.load_model("iekari/scripts/price_model.sav") # 変更
    result = predict_price.predict(dist_to_nearest_station =station, area =area, year_pp =year)

    return render(request, 'iekari/templates/iekari/assessment_result.html', {"station":station, "area":area, "year":year, 'result':int(result*area)}) # 変更
...
def submit_view(request): # [ REWRITE 3 ] 追記
    ...
    return HttpResponse(
        ...
        サイト管理者が承認を許可すると、<a href="/iekari/rentroom">RentRoom一覧</a>から提出いただいた物件を確認できます。</p>"""
        )
