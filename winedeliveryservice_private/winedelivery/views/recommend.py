...
from winedelivery.scripts.recommend_existing_user import Recommend_Existing
from winedelivery.scripts.recommend_new_user import RecommendNew

from winedelivery.models import RentRoom

def form_view(request): # フォームを表示
    return render(request, 'winedelivery/recommend_form.html') 
...
def result_view(request): # フォームの入力をAPIに伝達、リターン値を用いてテンプレートに表示
    ...
    return render(request, 'winedelivery/recommend_result.html', {'results':results})
